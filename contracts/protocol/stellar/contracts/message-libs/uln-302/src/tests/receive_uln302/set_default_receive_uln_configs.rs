...
fn test_set_default_receive_uln_configs() {
    ...
    env.mock_auths(&[MockAuth {
        ...
        invoke: &MockAuthInvoke {
            ...
            args: (&oapp_receive_uln_configs,).into_val(&env),
            sub_invokes: &[],
        },
    }]);
...
fn test_set_default_receive_uln_configs_authorization() {
...
#[test]
fn test_set_default_receive_uln_configs_assert_default_config() {
    let TestSetup { env, uln302, .. } = setup();
...
#[test]
fn test_default_receive_uln_config_not_found() {
    // Sui equivalent: test_get_default_config_not_found (from receive_uln_tests.move)
    ...
    let config = uln302_receive_client.default_receive_uln_config(&999);
